<%@ page import="com.induk.bucketlist.domain.Member" %>
    <%-- Created by IntelliJ IDEA. User: rlawl Date: 2022-05-12 Time: 오후 9:46 To change this template use File |
            ...
            <%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
                <%@ page contentType="text/html;charset=UTF-8" language="java" %>
                    ...
                    <head>
                        ...
                        <link rel="icon" href="<c:url value=" /favicon/favicon.ico" />" type="image/x-icon"
                        sizes="16x16">
                    ...
                    <body>
                        <header class="sticky top-0">
                            <nav
                                class="relative h-full px-6 xl:px-52 flex justify-between items-center bg-white drop-shadow-sm">
                                <a href="/bucketlist">
                                    <img src="/images/logo-s.png" alt="logo" class="w-40 h-auto m-1" />
                                </a>
                                <!-- Search Bar -->
                                <div class="flex-grow relative" x-data="{ showMenu : false }"
                                    @click.away="showMenu = false">
                                    <div class="bg-white rounded-full m-2 p-2 drop-shadow-md ">
                                        <input readonly type="text" placeholder="&#xF002;  Explore Idea"
                                    ...
                                    <div x-show="showMenu"
                                        ...
                                        <div id="search-category"
                                            ...
                                            <div data-category-name="travel" class="category-name" tabindex="-1">여행
                                            </div>
                                            <div data-category-name="challenge" class="category-name" tabindex="-1">도전
                                            </div>
                                            <div data-category-name="exprience" class="category-name" tabindex="-1">경험
                                            </div>
                                            <div data-category-name="skill" class="category-name" tabindex="-1">기술</div>
                                            <div data-category-name="education" class="category-name" tabindex="-1">교육
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <% Member member=(Member)session.getAttribute("member"); String
                                                first_name="" ; if(member.getSrc().isEmpty()){
                                                first_name=member.getName().substring(0,1).toUpperCase(); } else {
                                                first_name=member.getSrc(); } %>

                                                <!-- user menu -->
                                                <div class="mx-4 h-12 w-12 inline-block relative"
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${empty sessionScope.member.src}">
                                                            <div class="h-12 w-12 cursor-pointer rounded-full border-2 border-transparent bg-indigo bg-no-repeat bg-cover bg-center bg-blue-50"
                                                                ...
                                                                b :class="{ 'border-indigo-100' : showMenu }"
                                                                @click="showMenu = !showMenu">
                                                            </div>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <img src="<%=" /images/member/"%>${member.src}"
                                                            alt="profile"
                                                            class="h-12 w-12 cursor-pointer rounded-full border-2
                                                            border-transparent bg-indigo bg-no-repeat bg-cover bg-center
                                                            bg-blue-50"
                                                            style="
                                                            background-image: url('https://ui-avatars.com/api/?name=
                                                            <%=first_name%>&color=7F9CF5&background=EBF4FF');
                                                                " b :class="{ 'border-indigo-100' : showMenu }"
                                                                @click="showMenu = !showMenu">
                                                        </c:otherwise>
                                                    </c:choose>
                                                    ...
                                                    <div x-show="showMenu"
                                                        ...
                                                        x-transition:leave-end="opacity-0 scale-90 translate-y-1">
                                                        <div
                                                            class="p-6 flex items-center justify-center border-b border-gray-200 w-full flex-col">
                                                            <c:choose>
                                                                <c:when test="${empty sessionScope.member.src}">
                                                                ...
                                                                <c:otherwise>
                                                                    <img src="<%=" /images/member/"%>${member.src}"
                                                                    alt="profile"
                                                                    class="w-20 h-20 rounded-full flex items-center
                                                                    justify-center bg-indigo text-4xl text-white
                                                            ...
                                                            <div class="mt-6">
                                                                <a href="/bucketlist/members/edit"
